<%@ Page Title="Mis Reservas" Language="C#" MasterPageFile="~/Staff.Master" AutoEventWireup="true" CodeBehind="MisReservas.aspx.cs" Inherits="OfficeHoteling._MisReservas" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    <style type='text/css'>
     ...
     <div class="jumbotron">
            <h1>Mis Reservas</h1>
            <p class="lead">Aqui podra ver todas sus reservas, manejarlas y checkinear en ellas</p>
            <%--<p><a href="http://www.asp.net" class="btn btn-primary btn-lg">Crear Reserva &raquo;</a></p>--%>
                <asp:GridView ID="GVReservas" runat="server" Width="852px" BackColor="#CCCCCC" BorderColor="#999999" BorderStyle="Solid" BorderWidth="3px" CellPadding="4" CellSpacing="2" ForeColor="Black" OnRowCommand="GVReservas_RowCommand" AutoGenerateColumns="False">
                    <Columns>
                        <asp:BoundField DataField="FechaDesde" DataFormatString="{0:MM-dd-yyyy hh:mm tt}" HeaderText="Fecha Desde" SortExpression="FechaDesde" />
                        <asp:BoundField DataField="FechaHasta" HeaderText="Fecha Hasta" SortExpression="FechaHasta" />
                        <asp:BoundField DataField="EspacioDescripcion" HeaderText="Espacio" SortExpression="EspacioDescripcion" />
                        <asp:BoundField DataField="StaffId" HeaderText="Staff Id" SortExpression="StaffId" Visible="False" />
                        <asp:TemplateField HeaderText="Cancelar">   
                            <ItemTemplate>
                                <asp:ImageButton id="imgDeleteReserva" runat="server" CommandName="Inactivate" CommandArgument='<%# Bind("Id") %>' ImageUrl="~/images/delete_icon.png" Height="20px" Width="20px" OnClientClick="return confirm('Esta seguro que quiere cancelar la reserva ?')"></asp:ImageButton>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Checkin">   
                            <ItemTemplate>
                                <asp:ImageButton id="imgCheckin" runat="server" CommandName="Checkin" CommandArgument='<%# Bind("Id") %>' ImageUrl="~/images/Checkin_icon.png" Height="20px" Width="20px" OnClientClick="return confirm('Para continuar checkineando por favor haga click en OK')"></asp:ImageButton>
                                <%--<asp:HiddenField ID="hdnReservaId" runat="server" Value='<%# Bind("ReservaId") %>' />--%>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
                    <EmptyDataTemplate>
                        <em>No hay reservas creadas para el usuario.</em>
                    </EmptyDataTemplate>
