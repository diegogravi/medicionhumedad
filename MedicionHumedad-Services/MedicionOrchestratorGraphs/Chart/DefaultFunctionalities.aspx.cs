...
namespace WebSampleBrowser.Chart
{
    public partial class DefaultFunctionalities : System.Web.UI.Page
    {
        ...
        {
            List<LineChartData> data = await this.GetPredictionDataAsync(txtDiaComienzo.Text, txtHoraComienzo.Text, Convert.ToInt32(txtDiasSiguientes.Text));
        ...
        public async Task<List<LineChartData>> GetPredictionDataAsync(string dateStart, string hourStart, int repetitions)
        {
            var httpClient = new HttpClient();
            var apiUrl = "https://localhost:44399/api/";
            var urlBase = apiUrl + "Orchestrator/GetPredictionData";
            var url = string.Format("{0}?dateStart={1}&hourStart={2}&repetitions={3}", urlBase, dateStart, hourStart, repetitions);
